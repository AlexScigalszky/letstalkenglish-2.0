...
@Component({
  ...
  styleUrl: './random-topic.component.scss',
})
export class RandomTopicComponent {
  icebreakerQuestions = [
    ...
    'If you could have a magical power, but only one, what would it be?',
    "What's your go-to karaoke song?",
    'If you could attend any major event in history, which one would you choose?',
  ...
  current = signal(this.nextQuestion());

  getRandomQuestion() {
    this.current.set(this.nextQuestion());
  }

  nextQuestion() {
    const randomIndex = Math.floor(
      Math.random() * this.icebreakerQuestions.length
    );
    return this.icebreakerQuestions[randomIndex];
  }
}
